...
const SNIPPETS: Snippet[] = [
  {
    ...
    code: (
      <pre className="overflow-x-auto rounded-lg bg-zinc-900 p-4 font-mono text-sm leading-relaxed text-zinc-100">
...
export function Stage01CodeViewer() {
  return (
    <div className="p-6">
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-foreground">코드 뷰어</h3>
      ...
      <Tabs defaultValue="mutation">
        <TabsList className="mb-4 bg-zinc-900 light:bg-zinc-100">
          {SNIPPETS.map((s) => (
            <TabsTrigger
              key={s.id}
              value={s.id}
              className="data-[state=active]:bg-zinc-800 data-[state=active]:text-zinc-100 light:data-[state=active]:bg-white light:data-[state=active]:text-zinc-900"
            >
              {s.badge} {s.label}
            </TabsTrigger>
          ))}
        ...
        {SNIPPETS.map((s) => (
          <TabsContent key={s.id} value={s.id}>
            <div className="space-y-4">
              <div className="rounded-lg border border-zinc-800 bg-zinc-900/40 p-4 light:border-border light:bg-muted">
                <p className="text-sm font-medium text-zinc-200 light:text-foreground">
                  {s.badge} {s.label}
                </p>
                <p className="mt-1 text-sm text-zinc-400 light:text-muted-foreground">{s.description}</p>
              </div>

              {s.code}

              <div className="rounded-lg border border-zinc-800 p-3 light:border-border">
                <p className="text-xs font-semibold uppercase tracking-wider text-zinc-500 light:text-muted-foreground">
                  언제 쓰면 좋은가
                </p>
                <p className="mt-1 text-sm text-zinc-400 light:text-foreground">{s.when}</p>
              </div>
            </div>
          </TabsContent>
        ))}
