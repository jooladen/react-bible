...
```
모바일:
[메인 콘텐츠 - pb 없음, 스크롤 여백은 stage-layout spacer로 처리]
[하단 Bottom Sheet - 올라옴]
[하단 고정: ☰ 스테이지 목록 ↑]
...
- 메인 콘텐츠: pb 없음 (스크롤 여백은 stage-layout 내 spacer div로 처리)
- 데스크탑 토글 버튼: `{isDesktop && <button>}` JS 조건부 렌더링 (`hidden md:block` Tailwind v4 불가)

### 4.3 stage-layout.tsx 변경

모바일에서 학습완료/이전/다음 nav bar를 항상 하단에 고정하고, 콘텐츠 끝까지 스크롤 가능하도록 처리.
...
>
```
- Tailwind v4에서 `bottom-[45px]` arbitrary value가 모바일에서 오작동 → inline style로 대체
...
```
- content div 끝에 위치
- fixed nav(44px) + ☰(45px) = 89px 가림 → 96px spacer로 스크롤 공간 확보
- `pb-*` 클래스 대신 자식 div 사용 (iOS Safari pb 미인식 버그 우회)
...
- 실기기(Galaxy S23+): Vercel 배포 환경에서 검증 완료 ✅
- 빌드 에러 0건 ✅
